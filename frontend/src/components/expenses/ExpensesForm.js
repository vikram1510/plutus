...
const ExpensesForm = ({ data, friends, errors, onChange, onSubmit, onSplitChange }) => {
  return (
    <form onSubmit={onSubmit}>
      <div>
        <input id='description' placeholder=' ' onChange={onChange} />
        <label htmlFor='description'>Description</label>
        {errors.description && <div className='error-message'>{errors.description}</div>}
      </div>
      <div>
        <input id='amount' type='number' placeholder=' ' onChange={onChange} />
        ...
        {friends && friends.map(({ id, username }) => (
          <div key={id} className='debtor-wrapper'>
            ...
            <div>
              ...
              <input id={id} type={data.split_type === 'equal' ? 'checkbox' : 'number'} placeholder='0' onChange={onSplitChange} />
            </div>
          </div>
        ))}
      </div>
